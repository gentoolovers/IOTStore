...
int cgiMain(){
	...
	char strJson[128];
	if (Tbl_Device_ControlModify(_Tbl_Device_Control))
	{
		sprintf(strJson,"{\"jsn\":1}");

		int fifo_fd=open(FIFONAME_PATH,O_WRONLY);
		if(fifo_fd >= 0)
		{
			write(fifo_fd,&_Tbl_Device_Control.DeviceID,sizeof(int));
		}
		close(fifo_fd);
