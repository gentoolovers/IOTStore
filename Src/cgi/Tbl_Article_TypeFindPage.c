...
int cgiMain(){
	...
	int count=Tbl_Article_TypeFindPage(list,sCon,sSort,atoi(iPageSize),atoi(iCurrentPageIndex));
	char strJson[1024]="";
	char str[128];
	if (count>0)
	{
		...
		sprintf(str,"],\"iTotalPageCount\":%d}",Tbl_Article_TypeGetTotalPageCount(sCon,atoi(iPageSize)));
		strcat(strJson,str);
	}
	else
	{
